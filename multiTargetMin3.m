...
for cv=1:cvFold
    ...
    for iteration=1:maxIteration 
        ...
        for i=1:n
            ...
            tempError=zeros(1, length(labelTr{i}));
            for j=1:length(labelTr{i})
                target=zeros(1, outputDim);
                target(labelTr{i}(j))=1;
                tempError(j)=sum((outputOut-target).^2)*(0.5);
            end
            [~, minErrorLoc]=min(tempError);
            clear tempError;
           
            % Backpropagation 

            target=zeros(1, outputDim);
            target(labelTr{i}(minErrorLoc))=1;
            for k1=1:outputDim
                for k2=1:hiddenDim
